...
RUN apt-get update -q -q && \
  ...
  apt-get update -q -q && \
  apt-get install php8.1-cgi php8.1-cli php8.1-pgsql php8.1-mysql php8.1-gd adduser --yes --force-yes && \
  adduser --system --group fcgi-php --home /var/lib/php && \
  for file in /etc/php/8.1/mods-available/*.ini; do phpenmod $(basename -s .ini "$file"); done && \
  apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* ~/.cache ~/.npm

COPY ./etc/nginx /etc/nginx
COPY ./etc/service/php /etc/service/php
COPY ./php /etc/php/8.1
