...
RUN apt-get update -q -q && \
  ...
  apt-get update -q -q && \
  apt-get install php7.1-cgi php7.1-cli php7.1-pgsql php7.1-mysql php7.1-gd adduser --yes --force-yes && \
  adduser --system --group fcgi-php --home /var/lib/php && \
  for file in /etc/php/7.1/mods-available/*.ini; do phpenmod $(basename -s .ini "$file"); done && \
  apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* ~/.cache ~/.npm

COPY ./etc/nginx /etc/nginx
COPY ./etc/service/php /etc/service/php
COPY ./php /etc/php/7.1
